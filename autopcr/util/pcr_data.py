...
from ..constants import CHARA_NAME_OVERRIDE_PATH
...
CHARA_NAME = {
    ...
    1359: ["刹鬼", "サツキ", "Satsuki", "鬼娘", "鬼萝莉", "鬼妹", "皐月"],
    ...
    1383: ["贪吃佩可(阿斯特赖亚)", "ペコリーヌ(アストライア)", "Pecorine(Astrea)", "佩可莉姆(阿斯特莱亚)", "阿吃", "火吃", "神吃"],
...
_DEFAULT_CHARA_NAME = {unit_id: list(names) for unit_id, names in CHARA_NAME.items()}
_CHARA_NAME_OVERRIDE_MTIME = None


def _reload_chara_name_override() -> None:
    global CHARA_NAME, _CHARA_NAME_OVERRIDE_MTIME
    if not CHARA_NAME_OVERRIDE_PATH:
        return

    try:
        mtime = os.path.getmtime(CHARA_NAME_OVERRIDE_PATH)
        if mtime == _CHARA_NAME_OVERRIDE_MTIME:
            return
        with open(CHARA_NAME_OVERRIDE_PATH, "r", encoding="utf-8") as stream:
            override = json.load(stream)
        merged = {unit_id: list(names) for unit_id, names in _DEFAULT_CHARA_NAME.items()}
        for unit_id, names in override.items():
            unit_id = int(unit_id)
            if not isinstance(names, list):
                raise ValueError(f"character aliases for {unit_id} must be a list")
            merged[unit_id] = list(dict.fromkeys(str(name) for name in names))
            nickname = CHARA_NICKNAME.get(unit_id)
            if nickname and nickname not in merged[unit_id]:
                merged[unit_id].append(nickname)
        CHARA_NAME = merged
        _CHARA_NAME_OVERRIDE_MTIME = mtime
    except FileNotFoundError:
        CHARA_NAME = {unit_id: list(names) for unit_id, names in _DEFAULT_CHARA_NAME.items()}
        _CHARA_NAME_OVERRIDE_MTIME = None
    except Exception:
        logger.exception(f"failed to load character aliases: {CHARA_NAME_OVERRIDE_PATH}")


def get_id_from_name(name):
    _reload_chara_name_override()
    for unit_id, names in CHARA_NAME.items():
        if name in names:
            return unit_id
    return None
