...
class LabyrinthGuildConfig(SingleChoiceConfig):
    def candidate_display(self, guild_id: int):
        return LABYRINTH_GUILDS.get(guild_id, str(guild_id))
...
@description('【请勿传播！不要恶意占用服务器资源！】不断重开黎明界开局，直到刷出满足配置要求的路线，每次执行最大尝试次数300次，区域3/5第3格有遗物和事件两种，遗物固定200分，事件可能更高也可能更低，自行选择。')
@name('黎明界刷开局')
...
@LabyrinthGuildConfig('labyrinth_reroll_guild_id', '公会', 5, list(LABYRINTH_GUILDS.keys()))
...
class labyrinth_start_reroll(Module):
    ...
    def _find_area_route(self, area: int, map_list: List, area3_bosses: Set[int], area5_bosses: Set[int], third_block_type: str) -> Tuple[Optional[List], str]:
        ...
        def dfs(block, path: List, seen: Set[int]) -> Optional[List]:
            ...
            if column not in expected:
                return None
            if self._block_type(block) not in self._expected_block_types(area, column, third_block_type):
                return None
    ...
    def _find_routes(self, map_list: List, difficulty: int, area3_bosses: Set[int], area5_bosses: Set[int], third_block_type: str) -> Tuple[Optional[Dict[int, List]], str]:
        routes: Dict[int, List] = {}
        failures = []
        for area in self._target_areas(difficulty):
            route, reason = self._find_area_route(area, map_list, area3_bosses, area5_bosses, third_block_type)
            if not route:
    ...
    async def do_task(self, client: pcrclient):
        ...
        if difficulty > max_unlocked_difficulty:
            ...
            raise SkipError("未解锁所选黎明界难度")

        if top.enter_id:
            self._log("检测到已有黎明界开局，先退休。")
        ...
        for attempt in range(1, max_count + 1):
            ...
            routes, reason = self._find_routes(enter.map_list or [], difficulty, area3_bosses, area5_bosses, third_block_type)
            if routes:
                self._log(f"刷到完美路线，总尝试次数：{attempt}")
        ...
        raise AbortError(f"重开{max_count}次仍未刷到目标路线，请自行添加BOSS数！最后失败原因：{last_reason}")
