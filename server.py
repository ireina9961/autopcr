...
- {prefix}保存EX状态|保存ex状态 [昵称] 保存当前普通EX装备穿戴状态
- {prefix}恢复EX状态|恢复ex状态 [昵称] 恢复已保存的普通EX装备穿戴状态
...
@register_tool("保存ex状态", "ex_equip_state")
async def save_ex_equip_state_lowercase(botev: BotEvent):
    return {"ex_equip_state_action": "保存"}


@register_tool("恢复ex状态", "ex_equip_state")
async def restore_ex_equip_state_lowercase(botev: BotEvent):
    return {"ex_equip_state_action": "恢复"}
